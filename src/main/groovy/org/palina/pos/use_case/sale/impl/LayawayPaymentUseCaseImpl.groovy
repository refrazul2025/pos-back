...
class LayawayPaymentUseCaseImpl implements LayawayPaymentUseCase{
    ...
    LayawayPaymentUseCaseImpl(UpdateStockUseCase updateStockUseCase,
    ...
    GeneralResponseDto<SaleDto> execute(SaleDto sale) {
        ...
        if ( model?.saleType?.equalsIgnoreCase("APARTADO")){
            model.saleClosed = false
